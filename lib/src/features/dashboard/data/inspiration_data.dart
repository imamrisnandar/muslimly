class InspirationQuote {
  final String arabic;
  final String translation;
  final String source;

  const InspirationQuote({
    required this.arabic,
    required this.translation,
    required this.source,
  });
}

class InspirationData {
  static const List<InspirationQuote> quotes = [
    InspirationQuote(
      arabic: "فَإِنَّ مَعَ ٱلْعُسْرِ يُسْرًا\nإِنَّ مَعَ ٱلْعُسْرِ يُسْرًا",
      translation:
          "Karena sesungguhnya sesudah kesulitan itu ada kemudahan, sesungguhnya sesudah kesulitan itu ada kemudahan.",
      source: "QS. Al-Insyirah: 5-6",
    ),
    InspirationQuote(
      arabic: "لَا يُكَلِّفُ ٱللَّهُ نَفْسًا إِلَّا وُسْعَهَا",
      translation:
          "Allah tidak membebani seseorang melainkan sesuai dengan kesanggupannya.",
      source: "QS. Al-Baqarah: 286",
    ),
    InspirationQuote(
      arabic:
          "رَبَّنَآ ءَاتِنَا فِي ٱلدُّنْيَا حَسَنَةً وَفِي ٱلْأَخِرَةِ حَسَنَةً وَقِنَا عَذَابَ ٱلنَّارِ",
      translation:
          "Ya Tuhan kami, berilah kami kebaikan di dunia dan kebaikan di akhirat dan peliharalah kami dari siksa neraka.",
      source: "QS. Al-Baqarah: 201",
    ),
    InspirationQuote(
      arabic: "وَإِذَا سَأَلَكَ عِبَادِي عَنِّي فَإِنِّي قَرِيبٌ",
      translation:
          "Dan apabila hamba-hamba-Ku bertanya kepadamu tentang Aku, maka (jawablah), bahwasanya Aku adalah dekat.",
      source: "QS. Al-Baqarah: 186",
    ),
    InspirationQuote(
      arabic: "إِنَّ ٱللَّهَ مَعَ ٱلصَّٰبِرِينَ",
      translation: "Sesungguhnya Allah beserta orang-orang yang sabar.",
      source: "QS. Al-Baqarah: 153",
    ),
    InspirationQuote(
      arabic: "خَيْرُ النَّاسِ أَنْفَعُهُمْ لِلنَّاسِ",
      translation:
          "Sebaik-baik manusia adalah yang paling bermanfaat bagi manusia.",
      source: "HR. Ahmad",
    ),
    InspirationQuote(
      arabic: "مَنْ لَا يَرْحَمْ لَا يُرْحَمْ",
      translation: "Barangsiapa tidak menyayangi, maka tidak akan disayangi.",
      source: "HR. Bukhari",
    ),
    InspirationQuote(
      arabic: "إِنَّمَا الْأَعْمَالُ بِالنِّيَّاتِ",
      translation: "Sesungguhnya setiap amal itu tergantung pada niatnya.",
      source: "HR. Bukhari & Muslim",
    ),
    InspirationQuote(
      arabic: "الْكَلِمَةُ الطَّيِّبَةُ صَدَقَةٌ",
      translation: "Perkataan yang baik adalah sedekah.",
      source: "HR. Bukhari & Muslim",
    ),
    InspirationQuote(
      arabic: "تَبَسُّمُكَ فِي وَجْهِ أَخِيكَ لَكَ صَدَقَةٌ",
      translation: "Senyummu di hadapan saudaramu adalah sedekah bagimu.",
      source: "HR. Tirmidzi",
    ),
    InspirationQuote(
      arabic: "وَمَن يَتَّقِ ٱللَّهَ يَجْعَل لَّهُۥ مَخْرَجًا",
      translation:
          "Barangsiapa bertakwa kepada Allah niscaya Dia akan mengadakan baginya jalan keluar.",
      source: "QS. At-Talaq: 2",
    ),
    InspirationQuote(
      arabic: "وَيَرْزُقْهُ مِنْ حَيْثُ لَا يَحْتَسِبُ",
      translation:
          "Dan memberinya rezeki dari arah yang tiada disangka-sangkanya.",
      source: "QS. At-Talaq: 3",
    ),
    InspirationQuote(
      arabic: "لَئِن شَكَرْتُمْ لَأَزِيدَنَّكُمْ",
      translation:
          "Sesungguhnya jika kamu bersyukur, pasti Kami akan menambah (nikmat) kepadamu.",
      source: "QS. Ibrahim: 7",
    ),
    InspirationQuote(
      arabic: "أَلَا بِذِكْرِ ٱللَّهِ تَطْمَئِنُّ ٱلْقُلُوبُ",
      translation:
          "Ingatlah, hanya dengan mengingati Allah-lah hati menjadi tenteram.",
      source: "QS. Ar-Ra'd: 28",
    ),
    InspirationQuote(
      arabic: "اللَّهُمَّ اغْفِرْ لِي ذَنْبِي كُلَّهُ",
      translation: "Ya Allah, ampunilah seluruh dosaku.",
      source: "HR. Muslim",
    ),
    InspirationQuote(
      arabic: "اِتَّقِ اللَّهَ حَيْثُمَا كُنْتَ",
      translation: "Bertakwalah kepada Allah di mana saja kamu berada.",
      source: "HR. Tirmidzi",
    ),
    InspirationQuote(
      arabic: "الدِّينُ النَّصِيحَةُ",
      translation: "Agama itu adalah nasihat.",
      source: "HR. Muslim",
    ),
    InspirationQuote(
      arabic: "الطُّهُورُ شَطْرُ الإِيمَانِ",
      translation: "Kebersihan adalah sebagian dari iman.",
      source: "HR. Muslim",
    ),
    InspirationQuote(
      arabic: "لَا تَغْضَبْ وَلَكَ الْجَنَّةُ",
      translation: "Janganlah marah, maka bagimu surga.",
      source: "HR. Thabrani",
    ),
    InspirationQuote(
      arabic: "رِضَى الرَّبِّ فِي رِضَى الْوَالِدِ",
      translation: "Ridha Allah terletak pada ridha orang tua.",
      source: "HR. Tirmidzi",
    ),
  ];
}
